...
const TOGGLEABLE_TOOLS_ZH: Array<{ name: string; label: string; description: string }> = [
  ...
  { name: 'media_analysis', label: '图片/视频分析', description: '图片/视频内容分析（需要支持视觉输入的模型）' },
...
const TOGGLEABLE_TOOLS_EN: Array<{ name: string; label: string; description: string }> = [
  ...
  { name: 'media_analysis', label: 'Image/Video Analysis', description: 'Image/video content analysis with a vision-capable model' },
      ...
      {activeTab === 'image' && (
        <div className="space-y-4">
          ...
          <div>
            ...
            >
              <option value="deepbot">Gemini Image</option>
              <option value="deepbot-gpt">GPT Image</option>
            </select>
          ...
          <div>
            ...
            />
            <p className="mt-1 text-xs text-gray-500">
              {imageGenConfig.provider === 'deepbot' && (lang === 'zh' ? 'Gemini 兼容图片生成服务地址' : 'Gemini-compatible image generation API URL')}
              {imageGenConfig.provider === 'deepbot-gpt' && (lang === 'zh' ? 'OpenAI 兼容图片生成服务地址' : 'OpenAI-compatible image generation API URL')}
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="mt-1 text-xs text-gray-500">
              {lang === 'zh' ? '使用所选图片模型服务商提供的 API Key' : 'Use the API key from your selected image model provider'}
            </p>
          </div>
