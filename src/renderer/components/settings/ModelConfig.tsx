...
export function ModelConfig({ onClose, tabId }: ModelConfigProps) {
  ...
  const [config, setConfig] = useState<ModelConfig>({
    providerType: 'custom',
    providerId: 'custom',
    providerName: PROVIDER_PRESETS.custom.name,
    baseUrl: PROVIDER_PRESETS.custom.baseUrl,
    modelId: PROVIDER_PRESETS.custom.defaultModelId,
    modelId2: PROVIDER_PRESETS.custom.defaultModelId2,  // 设置快速模型默认值
    modelName: PROVIDER_PRESETS.custom.defaultModelId,
    apiType: PROVIDER_PRESETS.custom.apiType,
    apiKey: '',
  });
  ...
  const [showModelDropdown, setShowModelDropdown] = useState(false); // 预设模型下拉菜单
  ...
  const loadConfig = async () => {
    try {
      ...
      if (actualResult.success && actualResult.config) {
        const loadedConfig = {
          ...actualResult.config,
          providerType: actualResult.config.providerType || 'custom',
          apiType: actualResult.config.apiType || 'openai-completions',
        };

        // 旧供应商（deepbot/qwen/deepseek/minimax）自动映射为 custom
        if (['deepbot', 'qwen', 'deepseek', 'minimax'].includes(loadedConfig.providerType)) {
          if (loadedConfig.providerType === 'deepbot' && String(loadedConfig.baseUrl || '').includes('deepbot' + '.plus')) {
            loadedConfig.baseUrl = PROVIDER_PRESETS.custom.baseUrl;
            loadedConfig.modelId = PROVIDER_PRESETS.custom.defaultModelId;
            loadedConfig.modelName = PROVIDER_PRESETS.custom.defaultModelId;
            loadedConfig.modelId2 = PROVIDER_PRESETS.custom.defaultModelId2;
          }
          loadedConfig.providerType = 'custom';
          loadedConfig.providerId = 'custom';
          loadedConfig.providerName = PROVIDER_PRESETS.custom.name;
