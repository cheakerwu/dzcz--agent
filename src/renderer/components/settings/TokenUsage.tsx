    ...
    <div style={{ padding: '24px', maxWidth: '800px' }}>
      ...
      {activeTab === 'image' && (
        <>
          ...
          <p style={{ marginTop: '16px', fontSize: '12px', color: 'var(--settings-text-dim)' }}>
            {lang === 'zh' 
              ? <>统计数据为当前 Local Agent Terminal 实例生成的图片数量，按提供商分类统计。</>
              : <>Statistics show the number of images generated by the current Local Agent Terminal instance, categorized by provider.</>}
          </p>
        </>
      )}
    </div>
