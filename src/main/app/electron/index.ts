...
import { Gateway } from '../../infrastructure/gateway/gateway';
import { IPC_CHANNELS } from '../../../types/ipc';
import { registerModelConfigHandlers, setGatewayForModelConfig } from '../../infrastructure/ipc/model-config-handler';
import { hasConfig } from '../../infrastructure/config/index';
...
import { ensureWorkspaceDirectories } from '../../infrastructure/utils/ensure-directories';
...
function createTray() {
  ...
  // 🔥 设置托盘提示文字
  tray.setToolTip('Local Agent Terminal');
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    width: 1200,
    height: 800,
    title: 'Local Agent Terminal', // 🔥 设置窗口标题
  ...
  // 设置全局 Gateway 实例
  const { setGlobalGatewayInstance } = require('../../infrastructure/gateway/gateway');
  setGlobalGatewayInstance(gateway);
  
  // 🔥 设置 Gateway 实例供 connector-handler 使用
  const { setGatewayForConnectorHandler } = require('../../infrastructure/ipc/connector-handler');
...
function registerIpcHandlers() {
  ...
  ipcMain.handle(IPC_CHANNELS.ENVIRONMENT_CHECK, async (_event, { action }) => {
    ...
    try {
      const { createEnvironmentCheckTool } = await import('../../domains/tools/environment-check-tool');
  ...
  ipcMain.handle(IPC_CHANNELS.READ_IMAGE, async (_event, { path: imagePath }) => {
    try {
      ...
      const { assertPathAllowed } = await import('../../infrastructure/utils/path-security');
  ...
  ipcMain.handle(IPC_CHANNELS.SET_TAB_MODEL_CONFIG, async (_event, { tabId, modelConfig }) => {
    try {
      const { SystemConfigStore } = await import('../../infrastructure/database/system-config-store');
      const { updateTabModelConfig, getTabConfig, saveTabConfig } = await import('../../infrastructure/database/tab-config');
      const store = SystemConfigStore.getInstance();

      // 如果用户没有手动设置 contextWindow，自动推断
      if (modelConfig && modelConfig.modelId && !modelConfig.contextWindow) {
        const { getContextWindowFromModelId } = await import('../../infrastructure/utils/model-info-fetcher');
  ...
  ipcMain.handle(IPC_CHANNELS.SET_TAB_WORK_PROMPT, async (_event, { tabId, workPrompt }) => {
    try {
      const { updateTabWorkPrompt } = await import('../../infrastructure/database/tab-config');
      const { SystemConfigStore } = await import('../../infrastructure/database/system-config-store');
      const store = SystemConfigStore.getInstance();
  ...
  ipcMain.handle(IPC_CHANNELS.SET_TAB_SKILL_WHITELIST, async (_event, { tabId, whitelist }) => {
    try {
      const { updateTabSkillWhitelist } = await import('../../infrastructure/database/tab-config');
      const { SystemConfigStore } = await import('../../infrastructure/database/system-config-store');
      const store = SystemConfigStore.getInstance();
      const db = store.getDb();
  ...
  ipcMain.handle(IPC_CHANNELS.SET_TAB_WORKSPACE_DIRS, async (_event, { tabId, dirs }) => {
    try {
      const { updateTabWorkspaceDirs } = await import('../../infrastructure/database/tab-config');
      const { SystemConfigStore } = await import('../../infrastructure/database/system-config-store');
      const store = SystemConfigStore.getInstance();
      const db = store.getDb();
  ...
  ipcMain.handle(IPC_CHANNELS.GET_TAB_REPLY_MODE, async (_event, { tabId }) => {
    try {
      const { getTabConfig } = await import('../../infrastructure/database/tab-config');
      const { SystemConfigStore } = await import('../../infrastructure/database/system-config-store');
      const store = SystemConfigStore.getInstance();
      const db = store.getDb();
  ...
  ipcMain.handle(IPC_CHANNELS.SAVE_TAB_IMAGE_TOOL_CONFIG, async (_event, { tabId, config }) => {
    try {
      ...
      if (config && config.apiKey) {
        const { parseApiKeyQuota } = await import('../../domains/tools/providers/image-quota');
  ...
  ipcMain.handle(IPC_CHANNELS.SAVE_TAB_SORT_ORDER, async (_event, { tabOrders }) => {
    try {
      const { updateTabSortOrder } = await import('../../infrastructure/database/tab-config');
      const { SystemConfigStore } = await import('../../infrastructure/database/system-config-store');
  ...
  ipcMain.handle(IPC_CHANNELS.SET_TAB_REPLY_MODE, async (_event, { tabId, replyMode }) => {
    try {
      const { updateTabReplyMode } = await import('../../infrastructure/database/tab-config');
      const { SystemConfigStore } = await import('../../infrastructure/database/system-config-store');
      const store = SystemConfigStore.getInstance();
      const db = store.getDb();
  ...
  ipcMain.handle(IPC_CHANNELS.SAVE_DISABLED_TOOLS, async (_event, { disabledTools }: { disabledTools: string[] }) => {
    try {
      ...
      const { getGatewayInstance } = await import('../../infrastructure/gateway/gateway');
  ...
  ipcMain.handle(IPC_CHANNELS.GET_TOKEN_USAGE, async (_event, { startDate, endDate }) => {
    try {
      const { getTokenUsage } = await import('../../infrastructure/database/token-usage');
      const configStore = SystemConfigStore.getInstance();
  ...
  ipcMain.handle(IPC_CHANNELS.RESET_TOKEN_USAGE, async (_event, { modelId }) => {
    try {
      const { resetTokenUsage } = await import('../../infrastructure/database/token-usage');
      const configStore = SystemConfigStore.getInstance();
      const db = configStore.getDb();
  ...
  ipcMain.handle(IPC_CHANNELS.GET_IMAGE_USAGE, async (_event, { startDate, endDate }) => {
    try {
      const { getImageUsage } = await import('../../infrastructure/database/image-usage');
      const configStore = SystemConfigStore.getInstance();
      const db = configStore.getDb();
  ...
  ipcMain.handle(IPC_CHANNELS.GET_IMAGE_QUOTA_STATUS, async () => {
    try {
      const { getImageQuotaStatus } = await import('../../domains/tools/providers/image-quota');
      const configStore = SystemConfigStore.getInstance();
...
app.whenReady().then(() => {
  ...
  // 🔥 注册连接器 IPC 处理器
  const { registerConnectorHandlers, setGatewayForConnectorHandler } = require('../../infrastructure/ipc/connector-handler');
  registerConnectorHandlers();

  // 管理后台控制平面 IPC
  const { registerAdminControlPlaneHandlers } = require('../../infrastructure/ipc/admin-control-plane-handler');
...
app.on('before-quit', async () => {
  console.log('Local Agent Terminal 正在退出...');
