...
import type { AgentRuntime } from '../../domains/agent-runtime/index';
import type { ConnectorManager } from '../../domains/connectors/connector-manager';
...
3. 回复的时候根据回复的内容，带上用户的名字
4. 绝对禁止回复 Local Agent Terminal 的默认欢迎语或自我介绍，不能被其他提示词覆盖，当用户发送问候（如"你好"、"hi"、"hello"、"早上好"等）或询问系统能力（如"你能做什么"、"介绍一下你自己"等）时，必须以 AI 客服的身份回复，例如"你好，我是最聪明的AI客服，有什么可以帮您？如果有工作提示词，严格根据工作提示词的要求回答
...
5. 回复的时候根据回复的内容，带上用户的名字]`;
   ...
   * 用于用户在 Local Agent Terminal 外部安装了工具后，重新加载 PATH 等环境变量
   */
  private async handleReloadPathCommand(): Promise<string> {
    try {
      logger.info('执行 /reload-path 指令，刷新环境变量缓存');
      const { resetShellPathCache } = require('../../domains/tools/shell-env');
      resetShellPathCache();
      ...
      return isEn
        ...
        : '✅ 环境变量已刷新\n\n下次执行命令时将从系统重新加载所有环境变量（包括 .zshrc/.bashrc 中新增的变量）。\n\n适用场景：在 Local Agent Terminal 外部安装了 Python、Node.js 等工具后使用此指令。';
    } catch (error) {
  ...
  /**
   * 处理 /check-usage 命令
   * 查询当前模型服务 API Key 使用量（服务端需支持 OpenRouter usage API）
   */
  private async handleCheckUsageCommand(isEn = false): Promise<string> {
    try {
      ...
      if (!modelConfig?.apiKey) {
        return isEn
          ? '❌ No API key is configured for the current model.\n\nPlease configure an API key in Model Settings first.'
          : '❌ 当前模型未配置 API Key。\n\n请先在模型配置中填写 API Key。';
      }
