...
import type {
  ...
  UpsertExternalStoreMappingInput,
...
function mapCompany(row: Row): AdminCompany {
  return {
    id: row.id,
    name: row.name,
    status: row.status,
...
export class AdminControlPlaneService {
  ...
  buildPromptContextForConnectorSession(input: { connectorId: string; conversationId: string }): string {
    this.ensureSchema();
    return new MemoryGateway(this.db).buildPromptContext(input);
  }
