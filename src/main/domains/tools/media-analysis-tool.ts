...
import { TIMEOUTS } from '../../infrastructure/config/constants/timeouts';
...
export function createMediaAnalysisTool(configStore: SystemConfigStore): AgentTool {
  return {
    ...
    label: '多媒体分析',
    description:
      '分析图片或视频内容。上传文件后使用 AI 视觉模型进行内容理解和描述。支持图片(jpg/png/gif/webp)和视频(mp4/mov/avi/mkv)。复用主模型配置，需要主模型支持视觉理解。',
    ...
    execute: async (_toolCallId: string, args: unknown, signal?: AbortSignal) => {
      try {
        ...
        // 展开 ~ 路径
        const { expandHomePath } = require('../../infrastructure/utils/path-security');
...
export const mediaAnalysisToolPlugin: ToolPlugin = {
  metadata: {
    ...
    name: '多媒体分析',
    version: '1.0.0',
    description: '上传图片/视频并使用 AI 视觉模型分析内容（复用主模型配置）',
