...
```bash
# 运行安装脚本
cd deepbot/src/main/domains/tools/email-tool
./install.sh
```
...
邮件工具是 Local Agent Terminal 的**内置工具**，代码位于 `src/main/domains/tools/email-tool.ts`。

**关键设计**：
- ✅ 工具代码在项目中（`src/main/domains/tools/`）
...
1. **在 `src/main/domains/tools/` 创建工具文件**（如 `my-tool.ts`）
2. **实现 `ToolPlugin` 接口**
...
参考文件：
- `src/main/domains/tools/email-tool.ts` - 工具实现
- `src/main/domains/tools/registry/tool-loader.ts` - 工具加载
- `src/main/domains/tools/registry/example-tool.ts` - 示例模板
