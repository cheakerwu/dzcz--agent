...
import { callAI } from '../../infrastructure/utils/ai-client';
...
export class AgentRuntime {
  ...
  constructor(workspaceDir: string, sessionId?: string, modelConfigOverride?: any) {
    ...
    // 如果数据库中没有，使用模型 ID 推断
    if (!contextWindow) {
      const { getContextWindowFromModelId } = require('../../infrastructure/utils/model-info-fetcher');
  ...
  private async loadHistoryToContext(): Promise<void> {
      try {
        ...
        if (this.instanceManager.agent) {
          ...
          // 🔥 使用现有的上下文压缩功能
          const { manageContext } = await import('../../infrastructure/context/context-manager');
  ...
  private async initializeSystemPrompt(): Promise<void> {
    ...
    if (this.systemPromptInitializing) {
      ...
      const { TIMEOUTS } = await import('../../infrastructure/config/constants/timeouts');
  ...
  /**
   * 构建 Fast 模式精简提示词
   * 只包含：身份信息 + 结构化运营上下文 + 工作提示词（通过 transformContext 注入）
   */
  private async buildFastModePrompt(): Promise<string> {
    ...
    return buildFastModeSystemPrompt({
      agentName,
      userName: nameConfig.userName,
      adminMemoryContext,
    });
  }
