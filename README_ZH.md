...
Local Agent Terminal 是一个本地优先的 AI 助手工作台，面向研发、办公自动化和可扩展 Agent 工作流。它支持多 Tab Agent、文件操作、命令执行、浏览器自动化、定时任务、长期记忆和 Skill 扩展。你可以接入自己的模型提供商和工具服务，并按个人或团队需求继续二开。

### ✨ 核心特性

- 🎯 **多任务并行处理** - 同时处理多个任务，互不干扰
- 🔧 **20+ 内置工具** - 文件操作、命令执行、浏览器控制、图片生成、图片/视频分析、文档分析、AI 对话、跨会话通信、网页内容获取、飞书/微信/企业微信消息、飞书云文档操作等
- 🧠 **记忆系统** - 长期记忆用户偏好和重要信息
- ⏰ **定时任务** - 自动化执行周期性任务
...
Docker 部署适用于 Linux 服务器，可直接基于本仓库构建。
...
Local Agent Terminal 采用模块化架构，支持多 Agent 互相对话和协作：
...
Local Agent Terminal 支持通过 Connector 系统接入外部平台，实现跨平台交互。

### 已支持平台

#### 飞书 (Feishu)

通过飞书机器人与 Local Agent Terminal 交互，支持私聊和群聊。
...
1. 在 Local Agent Terminal 中打开「系统设置」→「外部连接」→「飞书」
...
通过微信与 Local Agent Terminal 交互，支持私聊和群聊。
...
通过企业微信应用与 Local Agent Terminal 交互。
...
| **Media Analysis** | 图片/视频分析 | 图片内容描述、视频理解、OCR 识别（需视觉模型支持） |
| **Doc Analysis** | 文档分析 | 读取 PDF/Word/Excel/PPT 等文档内容（需安装 markitdown） |
| **Web Search** | 网页搜索 | 实时信息查询、资料搜集、内容研究（Tavily API） |
...
| **Feishu Send** | 飞书消息 | 向飞书用户发送文本、图片、文件 |
| **Feishu Doc Tool** | 飞书云文档 | 创建文档、读取内容、追加/更新/删除块、添加评论 |
| **WeChat Send** | 微信消息 | 向微信用户发送文本、图片、文件 |
| **WeCom Send** | 企业微信消息 | 向企业微信用户发送文本、图片、文件 |
| **Smart KF Send** | 智能客服消息 | 向智能客服用户发送文本、图片、文件 |
| **System Command** | 系统指令 | 清空会话等系统级操作 |
...
**配置文件路径**（按优先级查找）：
1. 项目级别：`<workspace>/.local-agent/tools/email-tool/config.json`
2. 用户级别：`~/.local-agent/tools/email-tool/config.json`
...
- **配置文件**: 从 `~/.local-agent/tools/<tool-name>/config.json` 读取配置
...
Local Agent Terminal 实现了严格的安全限制，确保 AI Agent 只能访问用户明确授权的目录：
...
| **脚本目录** | `~/.local-agent/scripts` | Python 脚本存储 | ✅ |
| **Skill 目录** | `~/.agents/skills` | Skill 包安装 | ✅ |
| **图片目录** | `~/.local-agent/generated-images` | AI 生成图片保存 | ✅ |
...
Local Agent Terminal 支持强大的长期记忆功能，可以记住用户的偏好和重要信息。

### 全局记忆

- **存储位置**: `~/.local-agent/memory/MEMORY.md`
...
```
用户: "记住：我喜欢简洁的代码"
Local Agent Terminal: "已记住你的偏好"
```

**独立记忆**：
```
用户: "创建一个销售分析 Agent"
Local Agent Terminal: "已创建新 Tab，这个 Agent 将专注于客户关系管理和销售数据分析"
...
```
用户: "每天早上 9 点检查桌面文件"
Local Agent Terminal: "已创建定时任务，将在每天 9:00 执行"
```
...
```bash
# 在 Local Agent Terminal 中使用 Skill Manager 工具
"安装 weather skill"
```
...
将 Skill 目录放到 `~/.agents/skills/` 下，重启 Local Agent Terminal 即可自动加载。

**方式 2：使用 Skill Manager**

```bash
# 在 Local Agent Terminal 中使用命令
...
Local Agent Terminal 支持多种 AI 模型提供商：
...
Local Agent Terminal 针对标准对话模型进行了优化。带有内置思考/推理模式的模型（如通义千问的 QwQ 系列、OpenAI 的 o1 系列，或其他具有显式推理步骤的模型）可能会导致：
...
Local Agent Terminal 集成了以下外部服务：
...
Local Agent Terminal 的二开基于并受到以下项目启发：
