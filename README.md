...
<img src="banner.jpg" alt="Local Agent Terminal" width="500"/>

<p>

**A local-first AI agent workspace you can customize**
...
Local Agent Terminal is a local-first AI assistant workspace for development, office automation, and extensible agent workflows. It supports multi-tab agents, file operations, command execution, browser automation, scheduled tasks, long-term memory, and Skill extensions. You can connect your own model providers and tool services, then customize the app for private or team use.
...
git clone https://github.com/local-agent/local-agent.git
cd local-agent
...
Docker deployment is available for Linux servers and can be built from this repository.
...
```bash
sudo xattr -rd com.apple.quarantine /Applications/Local Agent Terminal.app
```
...
Local Agent Terminal uses a modular architecture with support for multi-Agent communication and collaboration:
...
Local Agent Terminal connects to external platforms via the Connector system for cross-platform interaction.

### Supported Platforms

#### Feishu (Lark)

Interact with Local Agent Terminal through a Feishu bot, supporting both direct messages and group chats.
...
1. In Local Agent Terminal, go to "System Settings" → "External Connections" → "Feishu"
...
Interact with Local Agent Terminal via WeChat, supporting private and group chats.
...
Interact with Local Agent Terminal via WeCom applications.
...
| **Media Analysis** | Image/video analysis | Image description, video understanding, OCR (Local Agent Terminal provider only) |
...
1. Project-level: `<workspace>/.local-agent/tools/email-tool/config.json`
2. User-level: `~/.local-agent/tools/email-tool/config.json`
...
- **Config files**: Read from `~/.local-agent/tools/<tool-name>/config.json`
...
Local Agent Terminal enforces strict access controls so AI Agents can only reach directories you've explicitly authorized.
...
| **Scripts** | `~/.local-agent/scripts` | Python script storage | ✅ |
| **Skills** | `~/.agents/skills` | Skill package installation | ✅ |
| **Images** | `~/.local-agent/generated-images` | AI-generated image storage | ✅ |
...
Local Agent Terminal supports persistent long-term memory to retain user preferences and important context across sessions.

### Global Memory

- **Location**: `~/.local-agent/memory/MEMORY.md`
...
Local Agent Terminal: "Got it, I'll keep that in mind"
```

**Per-tab memory**:
```
User: "Create a Sales Analysis Agent"
Local Agent Terminal: "New Tab created. This Agent will focus on customer relationship management and sales data analysis"
...
Local Agent Terminal: "Scheduled task created — will run daily at 9:00"
```
...
```bash
# Use the Skill Manager tool in Local Agent Terminal
...
Place the Skill directory under `~/.agents/skills/` and restart Local Agent Terminal — it will be loaded automatically.
...
Local Agent Terminal is optimized for standard conversational models. Models with built-in thinking/reasoning modes can cause display issues with `<think>...</think>` tags, slower responses, and unnecessary overhead for simple tasks.
...
Local Agent Terminal was adapted from and inspired by:
...
**Local Agent Terminal is ready for local development and customization.**
